...
SELECT
	...
	FORMAT(accounts.createdon, 'yyyy-MM-dd') As 'Created On'
FROM
(
...
ORDER BY map.label ASC, accounts.createdon DESC;
...
SELECT DISTINCT
	map.label AS 'Community Group',
	data.name AS NAME,
	data.industryname AS 'Industry Name',
	data.subindustryname AS 'SubIndustry Name'
FROM
(
...
(
	SELECT DISTINCT
		name,
		a.accountid AS id,
		a.socoro_subindustryname AS subindustryname,
		a.cr67a_industryname AS industryname,
